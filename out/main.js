...
coconut_diffing_Widget.prototype = {
	...
	,_coco_teardown: function() {
		...
		while(_g < _g1.length) {
			...
			case 0:
				var real = c.r;
				var _g2 = _this.unsetLastRender(real);
				if(_g2 != null) {
					var children = _g2.childList;
					var _g3 = 0;
...
coconut_diffing_Differ.prototype = {
	...
	,updateAll: function(before,nodes,parent,later) {
		...
		var after = this._renderAll(nodes,later,parent,{ native : native, widget : function(type1,key2,attr,widgetType) {
			...
			var _g23 = previous(coconut_diffing_Differ.WIDGET_INST,coconut_diffing__$Key_Key_$Impl_$.ofObject(w1));
			if(_g23 == null) {
...
coconut_vdom__$Child_Child_$Impl_$.ofView = function(v) {
...
var coconut_ui_Renderer = function(rendered,mounted,updated,unmounting) {
	coconut_diffing_Widget.call(this,rendered,mounted,updated,unmounting);
};
...
coconut_ui_Renderer.__super__ = coconut_diffing_Widget;
coconut_ui_Renderer.prototype = $extend(coconut_diffing_Widget.prototype,{
});
var coconut_ui_View = function(render,shouldUpdate,track,beforeRerender,mounted,updated) {
	...
	coconut_ui_Renderer.call(this,tink_state__$Observable_Observable_$Impl_$.auto(this1),mounted,function() {
...
coconut_ui_View.__super__ = coconut_ui_Renderer;
coconut_ui_View.prototype = $extend(coconut_ui_Renderer.prototype,{
...
App.prototype = $extend(coconut_ui_View.prototype,{
	render: function() {
		...
		var __r1 = [];
		var __ret = { };
		__r1.push(CocosDatePicker.fromHxx(__ret));
		var __ret1 = { id : "input-container"};
		__r.push(coconut_vdom__$Child_Child_$Impl_$.element("div",__ret1,__r1));
		return __r[0];
	}
	,__initAttributes: function(attributes) {
	}
});
var _$CocosDatePicker_PickerDayInfo_$Impl_$ = {};
_$CocosDatePicker_PickerDayInfo_$Impl_$.__name__ = true;
_$CocosDatePicker_PickerDayInfo_$Impl_$._new = function(value) {
	var this1 = value;
	return this1;
};
_$CocosDatePicker_PickerDayInfo_$Impl_$.from = function(value) {
	return _$CocosDatePicker_PickerDayInfo_$Impl_$._new({ monthDay : value.monthDay, weekDay : value.weekDay, month : value.month, year : value.year, foreign : false});
};
var CocosDatePicker = function(__coco_data_) {
	var _gthis = this;
	this.picker = new coconut_ui_Ref();
	this.input = new coconut_ui_Ref();
	var this1 = { f : function() {
		return new Date();
	}};
	this.__tink_defaults63 = { startDate : tink_state__$Observable_Observable_$Impl_$.auto(this1), culture : tink_state__$Observable_Observable_$Impl_$["const"]("en-GB")};
	this.__slots = { startDate : new coconut_ui_tools_Slot(this,null), culture : new coconut_ui_tools_Slot(this,null)};
	var this2 = new tink_state__$State_SimpleState(new Date(),null,null);
	this.__coco_currentDate = this2;
	var this3 = new tink_state__$State_SimpleState(0,null,null);
	this.__coco_selectedDayIndex = this3;
	this.__initAttributes(__coco_data_);
	var snapshot = null;
...
CocosDatePicker.__name__ = true;
CocosDatePicker.fromHxx = function(attributes) {
	return coconut_vdom__$Child_Child_$Impl_$.widget("CocosDatePicker",attributes.key,attributes.ref,attributes,{ create : function(__coco_data_) {
		return new CocosDatePicker(__coco_data_);
	}, update : function(attr,v) {
		v.__initAttributes(attr);
	}});
};
CocosDatePicker.__super__ = coconut_ui_View;
CocosDatePicker.prototype = $extend(coconut_ui_View.prototype,{
	addSvg: function() {
		var element = window.document.querySelector(".nav.left");
		element.innerHTML = "<svg version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">\n<path d=\"M 17,12 l -5,5 l 5,5\"></path>\n</svg>\n";
		element = window.document.querySelector(".nav.right");
	...
	,showPicker: function() {
		...
		var inputHeight = this.input.current.clientHeight + 3;
		this.picker.current.style.marginTop = "" + inputHeight + "px";
	}
	,onDayClick: function(day,idx) {
		this.__coco_selectedDayIndex.set(idx);
	}
	,onLeftClick: function() {
		var year = tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate).getFullYear();
		var month = tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate).getMonth();
		var param = new Date(year,month - 1,1,0,0,0);
		this.__coco_currentDate.set(param);
		console.log("src/CocosDatePicker.hx:174:",tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate));
	}
	...
	,onRightClick: function() {
		...
		var param = new Date(year,month + 1,1,0,0,0);
		this.__coco_currentDate.set(param);
		console.log("src/CocosDatePicker.hx:190:",tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate));
	}
	,getDays: function(cult) {
		var pickDays = [];
		var month = tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate).getMonth();
		var year = tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate).getFullYear();
		var days = DateManager.instance.getMonthDays(month,year);
		var firstDay = _$CocosDatePicker_PickerDayInfo_$Impl_$.from(days[0]);
		if(firstDay.weekDay > 0) {
			...
			var _g = 1;
			var _g1 = firstDay.weekDay + 1;
			while(_g < _g1) {
				var i = _g++;
				var pickDay = _$CocosDatePicker_PickerDayInfo_$Impl_$.from(lastDays[idx + i]);
				pickDay.foreign = true;
				pickDays.push(pickDay);
			}
		}
		var _g2 = 0;
		while(_g2 < days.length) {
			var day = days[_g2];
			++_g2;
			pickDays.push(_$CocosDatePicker_PickerDayInfo_$Impl_$.from(day));
		}
		var lastDay = _$CocosDatePicker_PickerDayInfo_$Impl_$.from(days[days.length - 1]);
		if(lastDay.weekDay < 6) {
			var len = 6 - lastDay.weekDay;
			...
			while(_g11 < _g21) {
				var i1 = _g11++;
				var pickDay1 = _$CocosDatePicker_PickerDayInfo_$Impl_$.from(nextDays[i1]);
				pickDay1.foreign = true;
				pickDays.push(pickDay1);
			}
		}
		return pickDays;
	}
	,render: function() {
		var _gthis = this;
		var cult = culture_CultureManager.instance.getCulture(tink_state__$Observable_Observable_$Impl_$.get_value(this.__slots.culture));
		var month = tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate).getMonth();
		var year = tink_state__$State_State_$Impl_$.get_value(this.__coco_currentDate).getFullYear();
		var monthName = cult.nameMonths[month];
		this.pickDays = this.getDays(cult);
		...
		var __r3 = [];
		var __ret4 = { className : tink_domspec__$ClassName_ClassName_$Impl_$.ofString("nav left"), onclick : tink_core__$Callback_Callback_$Impl_$.fromNiladic($bind(_gthis,_gthis.onLeftClick))};
		var __r4 = [];
		__r3.push(coconut_vdom__$Child_Child_$Impl_$.element("div",__ret4,__r4));
		var __ret5 = { className : tink_domspec__$ClassName_ClassName_$Impl_$.ofString("title"), onclick : tink_core__$Callback_Callback_$Impl_$.fromNiladic($bind(_gthis,_gthis.onTitleClick))};
		var __r5 = [];
		__r5.push(coconut_vdom__$Child_Child_$Impl_$.ofText(monthName));
		__r5.push(coconut_vdom__$Child_Child_$Impl_$.ofText(" "));
		__r5.push(coconut_vdom__$Child_Child_$Impl_$.ofInt(year));
		__r3.push(coconut_vdom__$Child_Child_$Impl_$.element("div",__ret5,__r5));
		var __ret6 = { className : tink_domspec__$ClassName_ClassName_$Impl_$.ofString("nav right"), onclick : tink_core__$Callback_Callback_$Impl_$.fromNiladic($bind(_gthis,_gthis.onRightClick))};
		var __r6 = [];
		...
		var __r10 = [];
		var _g1 = 0;
		var _g11 = this.pickDays.length;
		while(_g1 < _g11) {
			var i = [_g1++];
			var day1 = [this.pickDays[i[0]]];
			if(day1[0].foreign) {
				var __ret11 = { className : tink_domspec__$ClassName_ClassName_$Impl_$.ofString("day foreign"), onclick : (function(day2,i1) {
					return function(event) {
						_gthis.onDayClick(day2[0],i1[0]);
					};
				})(day1,i)};
				var __r11 = [];
				__r11.push(coconut_vdom__$Child_Child_$Impl_$.ofInt(day1[0].monthDay));
				__r10.push(coconut_vdom__$Child_Child_$Impl_$.element("div",__ret11,__r11));
			} else if(tink_state__$State_State_$Impl_$.get_value(this.__coco_selectedDayIndex) == i[0]) {
				var __ret12 = { className : tink_domspec__$ClassName_ClassName_$Impl_$.ofString("day selected")};
				var __r12 = [];
				__r12.push(coconut_vdom__$Child_Child_$Impl_$.ofInt(day1[0].monthDay));
				__r10.push(coconut_vdom__$Child_Child_$Impl_$.element("div",__ret12,__r12));
			} else {
				var __ret13 = { className : tink_domspec__$ClassName_ClassName_$Impl_$.ofString("day"), onclick : (function(day3,i2) {
					return function(event1) {
						_gthis.onDayClick(day3[0],i2[0]);
					};
				})(day1,i)};
				var __r13 = [];
				__r13.push(coconut_vdom__$Child_Child_$Impl_$.ofInt(day1[0].monthDay));
				__r10.push(coconut_vdom__$Child_Child_$Impl_$.element("div",__ret13,__r13));
			}
		}
	...
	,__initAttributes: function(attributes) {
		var this1 = attributes.startDate;
		this.__slots.startDate.setData(this1 == null ? this.__tink_defaults63.startDate : this1);
		var this2 = attributes.culture;
		this.__slots.culture.setData(this2 == null ? this.__tink_defaults63.culture : this2);
	}
});
var DateManager = function() {
};
DateManager.__name__ = true;
DateManager.prototype = {
	getMonthDays: function(month,year) {
		...
		while(_g < _g1) {
			var day = _g++;
			res.push({ monthDay : day, weekDay : weekDayOfFirst, month : month, year : year});
			++weekDayOfFirst;
			if(weekDayOfFirst > 6) {
				weekDayOfFirst = 0;
			}
...
Main.__name__ = true;
Main.main = function() {
	coconut_ui_Renderer.mount(window.document.body,coconut_vdom__$Child_Child_$Impl_$.ofView(new App({ })));
};
Math.__name__ = true;
...
var coconut_ui_Ref = function() {
	var _gthis = this;
	this.setter = function(value) {
		_gthis.current = value;
	};
};
coconut_ui_Ref.__name__ = true;
...
var culture_CultureManager = function() {
	this.cultures = new haxe_ds_StringMap();
	var v = { nameDaysShortest : ["Вс","Пн","Вт","Ср","Чт","Пт","Сб"], nameMonths : []};
	var _this = this.cultures;
	if(__map_reserved["ru-RU"] != null) {
		_this.setReserved("ru-RU",v);
	} else {
		_this.h["ru-RU"] = v;
	}
	var v1 = { nameDaysShortest : ["Su","Mo","Tu","We","Th","Fr","Sa"], nameMonths : ["January","February","March","April","May","June","July","August","September","October","November","December"]};
	var _this1 = this.cultures;
	if(__map_reserved["en-GB"] != null) {
		_this1.setReserved("en-GB",v1);
	} else {
		_this1.h["en-GB"] = v1;
	}
};
culture_CultureManager.__name__ = true;
culture_CultureManager.prototype = {
	getCulture: function(code) {
		var _this = this.cultures;
		var res = __map_reserved[code] != null ? _this.getReserved(code) : _this.h[code];
		if(res == null) {
			var _this1 = this.cultures;
			if(__map_reserved["en-GB"] != null) {
				return _this1.getReserved("en-GB");
			} else {
				return _this1.h["en-GB"];
			}
		}
		return res;
	}
...
culture_CultureManager.instance = new culture_CultureManager();
